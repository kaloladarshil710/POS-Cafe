...
// If no unpaid orders redirect back
if (empty($orders_arr)) {
    header("Location: index.php?msg=no_pending");
    exit();
}

// Collect all items + grand total
$all_items   = [];
$grand_total = 0;
$order_ids   = [];
...
.grand-row td{font-weight:800;font-size:15px;background:var(--primary-dim);color:var(--primary);border-radius:0 0 12px 12px;}
...
.pay-btn{width:100%;padding:16px;background:var(--primary);color:white;border:none;border-radius:14px;font-family:'Plus Jakarta Sans',sans-serif;font-size:15px;font-weight:800;cursor:pointer;transition:0.18s;letter-spacing:0.2px;}
.pay-btn:hover:not(:disabled){background:var(--primary-dark);transform:translateY(-2px);box-shadow:0 10px 28px rgba(249,115,22,0.3);}
.pay-btn:disabled{background:#D0D5DD;color:#98A2B3;cursor:not-allowed;}
...
.btn-confirm-upi{background:var(--green);border:none;border-radius:12px;padding:13px;font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;font-weight:700;color:white;cursor:pointer;}
...
<div class="page">
      ...
      <table>
        ...
        <tbody>
          ...
          foreach ($all_items as $item):
            // Show order separator if multiple orders
  ...
  <div class="pay-panel">
    ...
    <button class="pay-btn" id="payBtn" onclick="doPayment()">
      💳 Pay <?php echo $methods[0]['method_name'] ?? 'Now'; ?> — ₹<?php echo number_format($grand_total,2); ?>
    </button>

  </div>
</div>

<!-- Hidden forms for each non-UPI method -->
  ...
  <div class="modal">
    ...
    <div class="modal-btns">
      ...
      <form action="process_table_bill.php" method="POST" style="display:contents;">
        ...
        <button type="submit" class="btn-confirm-upi">✅ Payment Received</button>
      </form>
    </div>
  </div>
</div>
...
function doPayment() {
  if (selName === 'UPI') {
    ...
    document.getElementById('formMethod').value = selName;
    document.getElementById('payForm').submit();
  }
}
function closeUpi() { document.getElementById('upiModal').classList.remove('open'); }
document.getElementById('upiModal').addEventListener('click', e => { if(e.target===e.currentTarget) closeUpi(); });
</script>
</body>
</html>
