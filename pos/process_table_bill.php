...
foreach ($order_ids as $raw_oid) {
    ...
    // Get order amount
    $ord = mysqli_fetch_assoc(mysqli_query($conn, "SELECT total_amount FROM orders WHERE id=$oid"));
    if (!$ord) continue;
...
// Free the table + reset occupied_since
$free = mysqli_prepare($conn, "UPDATE restaurant_tables SET status='free', occupied_since=NULL WHERE id=?");
mysqli_stmt_bind_param($free, "i", $table_id);
mysqli_stmt_execute($free);
mysqli_stmt_close($free);

// Redirect to success page
$last_oid = intval(end($order_ids));
