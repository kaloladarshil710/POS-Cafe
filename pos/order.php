...
// FIXED: JOIN with categories to get category_name correctly
$products = mysqli_query($conn, "SELECT p.*, c.category_name
FROM products p
LEFT JOIN categories c ON p.category_id = c.id
ORDER BY c.category_name ASC, p.name ASC");

// FIXED: Get categories from categories table (not a broken DISTINCT on missing column)
$cats_q = mysqli_query($conn, "SELECT DISTINCT c.category_name FROM categories c
    ...
    <div class="left">
        ...
        <div class="products-area">
            <div class="product-grid" id="productGrid">
                ...
                <!-- FIXED: use category_name (from JOIN) instead of missing 'category' column -->
                <div class="product-card" data-cat="<?php echo htmlspecialchars($row['category_name']); ?>">
                    <div class="p-cat"><?php echo htmlspecialchars($row['category_name']); ?></div>
                    ...
                    <button class="add-btn" onclick="addToCart(<?php echo $row['id']; ?>, '<?php echo addslashes(htmlspecialchars($row['name'])); ?>', <?php echo floatval($row['price']); ?>)">
                        + Add to Order
                    </button>
                </div>
                <?php endwhile; ?>
